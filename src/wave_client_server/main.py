...
app = FastAPI(
    title="Wave Client Server",
    description="Mock server for Wave client integration testing",
)
...
async def create_session(session: SessionCreate):
    ...
    try:
        existing_session = (
            db.query(WaveSession)
            .filter(WaveSession.session_id == session.session_id)
            .first()
        )
        if existing_session:
...
async def get_session(session_id: str):
    ...
    try:
        session = (
            db.query(WaveSession).filter(WaveSession.session_id == session_id).first()
        )
        if not session:
...
async def create_message(message: MessageCreate):
    ...
    try:
        session = (
            db.query(WaveSession)
            .filter(WaveSession.session_id == message.session_id)
            .first()
        )
        if not session:
            raise HTTPException(status_code=404, detail="Session not found")
...
async def get_messages(session_id: str):
    ...
    try:
        ...
        messages = (
            db.query(WaveMessage).filter(WaveMessage.session_id == session_id).all()
        )
        return messages
    finally:
        db.close()
